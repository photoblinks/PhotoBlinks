...
/** The full location detail page body, shared by /location/[slug] and the
 * photographer share route /c/[token]/location/[slug].
 *
 * `showSponsoredPhotographer` is decided by the ROUTE that renders this
 * (hard-coded per route file), never by request input. When false, the
 * sponsored-photographer query is not made at all and the card — including
 * its impression/click analytics — is never mounted. */
export async function LocationDetailContent({
  location,
  showSponsoredPhotographer,
}: {
  location: PublicLocationDetail;
  showSponsoredPhotographer: boolean;
}) {
  ...
  const [
    ...
    getActiveCategories(),
    showSponsoredPhotographer && location.state_id
      ? getActiveSponsoredPhotographerByState(location.state_id)
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <aside className="flex flex-col gap-6">
          ...
          {showSponsoredPhotographer && sponsoredPhotographer && (
